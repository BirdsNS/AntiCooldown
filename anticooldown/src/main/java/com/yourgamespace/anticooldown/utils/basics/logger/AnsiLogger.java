...
public class AnsiLogger implements Logger {
    ...
    private static final Pattern RBG_TRANSLATE = Pattern.compile('§' + "x(" + '§' + "[A-F0-9]){6}", Pattern.CASE_INSENSITIVE);
    ...
    public void info(String message) {
        AntiCooldown.getInstance().getLogger().info(getMessage(message));
    }

    @Override
    public void warn(String message) {
        AntiCooldown.getInstance().getLogger().warning(getMessage(message));
    }
    ...
    private String getMessage(String message) {
        ...
        for (int var4 = 0; var4 < var5; ++var4) {
            ...
            result = result.replaceAll("(?i)" + color.toString(), this.replacements.getOrDefault(color, ""));
        }
